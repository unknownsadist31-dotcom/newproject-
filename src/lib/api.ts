...
export const getDexScreenerTokens = async (tokenAddresses: string[], chainId: 'solana' | 'ethereum' = 'solana'): Promise<Record<string, { price?: number; logo?: string }>> => {
  ...
  try {
    const res = await axios.get(`/api/proxy/dexscreener/tokens/v1/${chainId}/${addresses}`)
...
const thornode = axios.create({ baseURL: '/api/proxy/lq-thornode' })
const midgard = axios.create({ baseURL: '/api/proxy/lq-midgard' })
const mayaMidgard = axios.create({ baseURL: '/api/proxy/mayamidgard' })
const mayanode = axios.create({ baseURL: '/api/proxy/mayanode' })
